...
def blockproc(im, blocksize, fun):
    xblocks = np.split(im, range(blocksize[0], im.shape[0], blocksize[0]), aixs=2)
    xblocks_proc = []
    for xb in xblocks:
    ...
    proc = np.concatenate(xblocks_proc, axis=2)
...
def imfilter_np(x, k):
    ...
    k: kernel, cx1xhxw
    '''
    x = ndimage.filters.convolve(x, np.expand_dims(k, 0), mode='wrap')
    return x
